...
namespace E_Shift
{
    public partial class frmDailyEndOfTheLoadsReport : Form
    {
        public frmDailyEndOfTheLoadsReport()
        {
            InitializeComponent();
        }

        // Method to load the report data
        private void LoadDailyEndOfTheLoadsReport()
        {
            using (SqlConnection connection = new SqlConnection(DbConnection.ConnectionString)) // Assuming DbConnection.ConnectionString provides your connection string
            {
                try
                {
                    ...
                    // Your SQL query to fetch daily end of loads data
                    SqlCommand command = new SqlCommand(@"
                        SELECT
                            ...
                            J.CompletionDate AS DeliveryDate,
                            J.JobID,
                    ...
                    rvwDailyEndOfTheLoadsReport.LocalReport.DataSources.Clear();
                    ...
                    rvwDailyEndOfTheLoadsReport.LocalReport.ReportPath = "C:\\Adhithya\\TOP-UP\\Assignments\\07.20_AD_Assignment-01\\AD-Final\\E-Shift\\E-Shift\\bin\\Debug\\DailyEndOfTheLoadsReport.rdlc"; // Recommended relative path

                    // Add the data source to the report viewer
                    rvwDailyEndOfTheLoadsReport.LocalReport.DataSources.Add(reportDataSource);

                    // Refresh the report to display the data
                    rvwDailyEndOfTheLoadsReport.RefreshReport();
                }
        ...
        private void lblDailyEndOfTheLoadsReport_Click(object sender, EventArgs e)
        {

        }

        private void frmDailyEndOfTheLoadsReport_Load(object sender, EventArgs e)
        {
            // Call the method to load data and set up the report viewer when the form loads
            LoadDailyEndOfTheLoadsReport();

            // This RefreshReport call might be redundant if already called at the end of LoadDailyEndOfTheLoadsReport()
            // but it doesn't hurt to keep it for certainty.
            this.rvwDailyEndOfTheLoadsReport.RefreshReport();
        }

        private void rvwDailyEndOfTheLoadsReport_Load(object sender, EventArgs e)
        {
